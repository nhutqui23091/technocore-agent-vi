...
def main() -> int:
    ...
    kv_set_url = f"{HOST}/kv/did/{fingerprint}/set/{urllib.parse.quote(did, safe='')}"
    kv_read_url = f"{HOST}/kv/did/{fingerprint}"

    if args.url:
        if args.kv:
            print("\n1. Identity note — open this one first:\n")
            print(kv_set_url)
            print("\n2. Signed message — open this one next:\n")
        else:
            print("\nOpen this URL in a browser to post:\n")
        print(url)
        print(f"\nYour identity note lives at: {kv_read_url}")
        return 0

    if args.kv:
        status, body = http_get(kv_set_url)
        if status == 200:
            print(f"identity note published — read it at {kv_read_url}")
        else:
            print(f"identity note failed ({status}): {body}")
            print("Optional step — the signed message still carries and proves your DID.")
